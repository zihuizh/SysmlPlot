...
状态：已完成第一轮原型，**图库框架路线已确定，具体框架尚未选定**。

目的：用同一份视图产物对比两个候选图库，验证端口、嵌套、仓格、大图和基础交互。
本轮为了控制变量沿用了 Java 布局；后续试验应改用候选框架自身的布局、交互和动画能力。
原型页放在 `build/proto/`（不进仓库）。
...
第一轮建议：下一轮优先继续验证 G6，因为它的端口和嵌套能力更接近当前需求；
Cytoscape 保留为对照。这个建议不是最终选型结论。
体积这一条可以靠后续优化补：tree-shaking 构建（估计能砍掉一半以上）或 gzip + `DecompressionStream` 内联。

## 下一轮验证

1. 使用框架原生布局，不再依赖 Java 坐标。
2. 验证缩放、拖动、选中、聚焦、筛选和视图切换。
3. 验证端口、嵌套节点和仓格的最终表现。
4. 验证重排与视图切换动画是否能帮助跟踪元素。
5. 记录首帧、重排、内存和打包体积，再做最终选型。

...
1. **语义与展示分离**：产物不带坐标、不带尺寸、不带颜色。布局、样式、交互和动画由展示层负责，
   可丢弃、可重算，也可以按需另存为展示状态。
...
因为产物是语义层，同一个产物可以交给图库框架做交互展示，也可以交给现有无头渲染器做
确定性导出。具体图库尚未选定，框架专有数据由展示层适配，不进入本契约。
...
展示层消费视图产物，并把用户操作转换为稳定的语义引用：

```text
render(product, displayState?)  → 视图
onSelect(nodeId | null)         → 语义引用
onNavigate(nodeId, action)      → 导航请求
export(format)                  → SVG / PNG 等导出结果
```

约束（与 Spec42 的做法一致）：

- 展示层负责布局、样式、交互和动画；不许从名字、标签或位置推断语义关系
- 展示层不得修改产物，也不提供模型回写
- 框架专有坐标、样式和运行状态留在展示层
- 布局失败时必须给出明确提示或降级展示，不能静默显示空图

### 6.1 现有 layout.json v0

`layout.json` 是现有 Java/HTML 渲染器的可选布局快照。它继续用于回归和兼容，
但不是新展示层的统一布局格式，也不要求图库框架以它作为运行时输入。
...
这套实现证明了视图产物足以支持基础展示和交互。后续不再继续扩展其中的自研布局；
新的布局、交互和动画将在图库框架内实现。
...
图库框架展示层直接消费同一份视图产物，通过独立适配器生成框架节点、边和容器数据。
布局、缩放、拖动、筛选、聚焦、视图切换及其动画都在框架侧完成。

已确定采用图库框架，但具体框架仍在试验。选型过程和当前结果见 `docs/GRAPH-LIB-P0.md`。
